<!-- 由 scripts/sync-upstream.sh 从 mowenxd/cli 的 skills/mo-tag/SKILL.md 生成，请勿手工编辑。 -->
...
* 用户要“从已有标签里选出 X 个标签，再给某篇笔记打标签/改标签”时，先使用 `mocli tag mine` 获取候选标签，再结合 [`mo-note`](note.md) 的 `mocli note tag` 完成写入。
* 用户没有说明标签类型时，默认按笔记标签处理，即使用 `--filter note` 或省略 `--filter`。
* 用户要“给某篇笔记加标签/删除某篇笔记标签/覆盖某篇笔记标签”时，不使用本技能，改用 [`mo-note`](note.md) 的 `mocli note tag`。
* 用户要“查看某个/某些标签下的笔记、按标签筛选笔记”时，不使用本技能，改用 [`mo-note`](note.md) 的 `mocli notes tagged`；需要先从已有标签中挑选时，先用 `mocli tag mine` 获取候选标签的名称和 ID。
...
当用户希望“从已有标签中选 X 个标签给某篇笔记打标签”时，按以下流程处理：
  ...
  * 用户确认标签后，切换到 [`mo-note`](note.md) 执行 `mocli note tag`。用户说“打上/追加这些标签”时用 `--append`；用户说“标签就设为这些/只保留这些”时用 `--reset`。
