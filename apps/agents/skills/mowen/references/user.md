<!-- 由 scripts/sync-upstream.sh 从 mowenxd/cli 的 skills/mo-user/SKILL.md 生成，请勿手工编辑。 -->
...
### 使用时机
  ...
  * 如果用户只提供昵称、姓名或备注名，先按 [`mo-shared`](../SKILL.md) 的通用流程解析 UID：优先使用 [`mo-remark`](remark.md)，未命中时再使用 `mocli user search` 或询问用户补充 UID。
...
| 字段 | 类型 | 说明 | 引用字段 |
|------|------|------|------|
| `info` | UserInfo | 目标用户基础信息。 | [`UserInfo - 用户信息`](output-schema.md#user-info) |
| `counter` | object | 用户计数信息，包含关注数、粉丝数、公开笔记数、付费笔记数和合集数。 | - |
| `note_idx` | object | 用户主页少量笔记样本的分类索引。仅使用 `--note` 时返回；每个分类通常只返回前若干条，不代表完整列表。 | - |
| `notes` | map[string]NoteInfo | 笔记样本信息 `Map`，键为笔记 ID，值为笔记信息。仅使用 `--note` 时返回，用于辅助判断创作方向。 | [`NoteInfo - 笔记信息`](output-schema.md#note-info) |
...
### 使用时机
  ...
  * 当其它技能需要 UID，而 [`mo-remark`](remark.md) 未命中备注时，可用本命令辅助查找候选用户。
...
| `users` | map[string]UserInfo | 用户信息 `Map`，键为用户 UID，值为用户信息。可基于 `uids` 做有序展示，或用于快速按 UID 查询用户详情。 | [`UserInfo - 用户信息`](output-schema.md#user-info) |
