<!-- 由 scripts/sync-upstream.sh 从 mowenxd/cli 的 skills/mo-discover/SKILL.md 生成，请勿手工编辑。 -->
...
| `notes` | map[string]NoteInfo | 笔记信息 `Map`，键为笔记 ID。动态详情中出现 `note_id`、`target_id`、`joins` 等代表笔记 id 的字段时，可用该字段补齐笔记标题、摘要、链接等信息。 | [`NoteInfo - 笔记信息`](output-schema.md#note-info) |
| `users` | map[string]UserInfo | 用户信息 `Map`，键为用户 UID。可用于补齐事件发起人、笔记作者等用户信息。 | [`UserInfo - 用户信息`](output-schema.md#user-info) |
