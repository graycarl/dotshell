...
When returning to an orchestrator or directly to the user:

- Open with a one-sentence answer.
- Follow with a short paragraph or bullet points as needed.
- Cite sources inline.
- Keep the response to roughly one screen of text. If the material is too long, write a file to `./exa-results/<topic>-<YYYY-MM-DD>.md` and point to it.
- End subagent responses with `sources_reviewed: N`.
