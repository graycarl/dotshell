...
After running searches, you often need to extract structured information from the results. This file covers how to do that well with the local Exa helper.

## When You Have Enough from Snippets

Exa search results include titles, URLs, and text snippets (when `--text` or `--highlights` is used). For many fields (company name, person name, funding round, publication date), the snippet is sufficient. Extract directly from what you have before fetching full pages.

## When to Deep-Read

Fetch the full page with the `contents` subcommand when:
...
```bash
./scripts/search.py contents --url https://source-1.com --url https://source-2.com --text
```

Batch up to 5-10 URLs per fetch call to minimize round trips. Avoid truncating the text; the point is to understand full page context.
...
4. **Negation fields** ("no review mentions X", "no Series A announcement"): These require checking that something is absent. Search for the positive case; if nothing surfaces, report absence with a confidence level based on how thorough your coverage was.
...
- `--summary-query` — ask Exa to summarize each result around a specific question.
- `--summary-schema` — request a structured summary matching a JSON schema.
- `--text` — include full page text for extraction.
