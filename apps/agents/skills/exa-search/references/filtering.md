...
1. Read the relevant content (use `contents` if snippets are insufficient).
...
- `--start-published-date` / `--end-published-date` — apply hard date filters server-side.
- `--include-text` / `--exclude-text` — filter results by required or forbidden phrases.
- `--text` — fetch full text for soft-filter judgment.
- `--max-age-hours` — discard stale sources by forcing recent livecrawls when needed.
